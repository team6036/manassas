...
public class SwerveCommand extends CommandBase {
    ...
    @Override
    public void execute() {
        if (RobotContainer.getPrimaryJoystick().getRawButton(11)) {
            SwerveModule[] modules = subsystem.getModules();
            for (int i = 0; i < modules.length; i++) {
                modules[i].setTargetVelocity(modules[i].getCurrentVelocity(), 0);
            }
            ...
            double rotation = RobotContainer.getPrimaryJoystick().getRawAxis(2);
