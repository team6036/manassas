...
public class Mk2SwerveModuleBuilder {
    ...
    private static final PidConstants DEFAULT_FALCON_ANGLE_CONSTANTS = new PidConstants(0.01, 0.0, 0.01);
    ...
    public Mk2SwerveModuleBuilder angleEncoder(CANCoder encoder, double offset) {
        angleSupplier = () -> {
            double angle = ((encoder.getAbsolutePosition() - offset) * Math.PI / 180);
            angle %= 2.0 * Math.PI;
            if (angle < 0.0) {
                angle += 2.0 * Math.PI;
            }
